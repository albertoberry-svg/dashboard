...
- **Botón Herramientas en el header:** clase `.pdf-download-btn` (mismo estilo), es un `<a>` con `href="https://sebastiangil.app/herramientas" target="_blank" rel="noopener"`. Contiene un SVG de llave inglesa y un `<span class="pdf-tooltip">Herramientas</span>`. Está situado entre el botón PDF y el toggle de tema. **NO eliminar ni mover.**
...
```
(EUR/USD widget: presente en el archivo live de GitHub — id: fx-*)
