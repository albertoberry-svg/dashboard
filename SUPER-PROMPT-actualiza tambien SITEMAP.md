...
1. **Ticker superior** → todos los valores del ticker. Los nombres de los indicadores se muestran en amarillo `#f0c040` en tema oscuro (clase `.ticker-name`), neutro en tema claro. No cambiar este estilo.
...
17. **`<meta name="description">`** en el `<head>` → actualiza si cambia el mes/año del dashboard

18. **`dateModified` en el JSON-LD** → actualiza con el mes y año actuales (formato `YYYY-MM`)

19. **`<lastmod>` en `sitemap.xml`** → actualiza con la fecha actual (formato `YYYY-MM-DD`) y sube el archivo al repo de GitHub junto con el `index.html`

20. **Texto "Actualizado: [mes] [año]"** en el pie de página → pon el mes y año actuales
...
**ARCHIVOS**
- Dashboard: `index.html` → subir a raíz del repo de GitHub
- Sitemap: `sitemap.xml` → subir a raíz del repo de GitHub (actualizar `<lastmod>` en cada actualización)
- Ambos archivos van juntos en cada commit

**ESTRUCTURA**
- HTML single-file con Chart.js
- **8 pestañas**: Resumen · Ciudades · Alquiler · Inversión · Macro/IPC · 🏦 Hipotecas · 🦹 Jetas · 📡 Noticias
- **Noticias siempre es la última pestaña** (posición 8)
- Orden en el array de `switchTab`: `['overview','ciudades','alquiler','inversion','macro','hipotecas','jetas','noticias']`

**SEO — NO TOCAR ESTAS LÍNEAS DEL `<head>`**
```html
...
```
⚠️ La etiqueta `google-site-verification` nunca debe eliminarse — es la verificación de Google Search Console.

**SÍ ACTUALIZAR en cada publicación:**
- `<meta name="description">` → incluir mes y año actuales
- `dateModified` en el bloque JSON-LD → formato `YYYY-MM`
- `<lastmod>` en `sitemap.xml` → formato `YYYY-MM-DD`
...
- Canvas IDs con prefijo `hip` para evitar duplicados: `hipChartEuribor`, `hipChartTiposEntidad`, `hipChartCuotaMensual`, `hipChartEsfuerzo`
- Contiene: 5 KPIs · 4 gráficos · tabla comparativa entidades · 3 cajas informativas
...
- Ticker nombres (`.ticker-name`): amarillo `#f0c040` en tema oscuro, `var(--text3)` en tema claro
...
- Noticias hora (`.news-time`) y fuente (`.news-source-tag`): amarillo `#f0c040` en tema oscuro, `var(--text3)` en tema claro

**PUBLICACIÓN**
- URL: **https://dashboard.sebastiangil.app**
- GitHub Pages, repo `albertoberry-svg/dashboard`, rama `main`, archivos en raíz
- Google Search Console verificado · Sitemap enviado · Indexación solicitada (8 mar 2026)
...
- Sitemap: `/mnt/user-data/outputs/sitemap.xml`
